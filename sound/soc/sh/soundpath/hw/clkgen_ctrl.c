	...
	for (iCnt = 0; ARRAY_SIZE(g_clkgen_ctrl_func_tbl) > iCnt; iCnt++) {
		...
		if (uiValue == g_clkgen_ctrl_func_tbl[iCnt].uiValue) {
			...
			if (NULL != g_clkgen_ctrl_func_tbl[iCnt].func) {
				...
				ret = audio_ctrl_func(SNDP_HW_CLKGEN, STAT_ON, 1);
				if (ERROR_NONE != ret)
					return ret;
...
void clkgen_stop(void)
{
	...
	if (audio_ctrl_func(SNDP_HW_CLKGEN, STAT_OFF, 1) != ERROR_NONE)
		return;
	sndp_log_debug_func("end\n");
}
...
{
	/* Clock framework API, Status ON */
	if (audio_ctrl_func(SNDP_HW_CLKGEN, STAT_ON, 1) != ERROR_NONE)
		return;
