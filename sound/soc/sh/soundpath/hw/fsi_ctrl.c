...
/*	  Reg					Value		D  C */
	...
	/* LRS:Clock inverted, BRS:Clock inverted */
	...
	for (iCnt = 0; ARRAY_SIZE(g_fsi_func_tbl) > iCnt; iCnt++) {
		...
		if (uiValue == g_fsi_func_tbl[iCnt].uiValue) {
			...
			if (NULL != g_fsi_func_tbl[iCnt].func) {
				...
				ret = audio_ctrl_func(SNDP_HW_FSI, STAT_ON, uiRegClr);
				if (ERROR_NONE != ret)
					return ret;
...
void fsi_stop(const u_int regclr)
{
	...
	if (audio_ctrl_func(SNDP_HW_FSI, STAT_OFF, regclr) != ERROR_NONE)
		return;
	/* PCM clear */
...
void fsi_soft_reset(void)
{
	...
	if (audio_ctrl_func(SNDP_HW_FSI, STAT_ON, 1) != ERROR_NONE)
		return;
	if (audio_ctrl_func(SNDP_HW_FSI, STAT_OFF, 1) != ERROR_NONE)
		return;
	sndp_log_debug_func("end\n");
}
...
int fsi_test_init_a()
{
	...
	/* Clock framework API, Status ON */
	ret = audio_ctrl_func(SNDP_HW_FSI, STAT_ON, 1);
	if (ret != ERROR_NONE)
		return ret;
