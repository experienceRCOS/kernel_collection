...
/*!
   ...
   @retval	EINVAL if Register Base Address Variable is NULL
		ERROR_NONE if success
*/
int audio_ctrl_func(enum sndp_hw_audio drv, int stat, const u_int regclr)
{
	/* Local variable declaration */
	struct clk *clk;
	unsigned long flags;
...
#endif
	if (!g_common_ulSrstRegBase) {
		sndp_log_err("Software Reset register is NULL\n");
		return -EINVAL;
	}
	switch (drv) {
	case SNDP_HW_CLKGEN:
...
/*!
   ...
   @param[out]	none
   @retval	EINVAL if the Register Base Address Variable is NULL
		ERROR_NONE if success
*/
int common_set_pll22(const u_int uiValue, int stat, u_int rate)
{
	/* Local variable declaration */
	u_int dev;
	...
	sndp_log_debug_func("start\n");
	if (!g_common_ulClkRstRegBase) {
		sndp_log_err("CPGA register ioremap is NULL\n");
		return -EINVAL;
	}
	/* Device check */
...
/*!
   ...
   @retval	EINVAL if the Register Base Address Variable is NULL
		ERROR_NONE if success
*/
int common_set_fsi2cr(u_int dev, int stat)
{
	/* Local variable declaration */
	int ret;
	unsigned long flags;
	...
	if (!g_common_ulClkGpioRegBase) {
		sndp_log_err("GPIO register ioremap is NULL\n");
		return -EINVAL;
	}
