...
#define ID_DFR0_PMUV2_NOT_SUPPORTED	0
#define ID_DFR0_PMUV1			1
#define ID_DFR0_PMUV2			2
...
struct pmu_regs {
	...
	unsigned int pmxevtyper[ARMV7_MAX_COUNTERS];
	unsigned int pmxevcntr[ARMV7_MAX_COUNTERS];
	unsigned int pmuserenr;
};
static DEFINE_PER_CPU(struct pmu_regs, pmu_regs);
static u32 pmu_ext;
...
static void armv7pmu_save_state(struct arm_pmu *cpu_pmu)
{
	u32 val;
	unsigned int cnt;
	struct pmu_regs *regs;

	if (atomic_read(&cpu_pmu->active_events) == 0)
		return;

	regs = &get_cpu_var(pmu_regs);

	/* save control register */
	asm volatile("mrc p15, 0, %0, c9, c12, 0" : "=r" (regs->pmcr));

	/* stop perf */
	armv7pmu_stop(cpu_pmu);

	asm volatile("mrc p15, 0, %0, c9, c12, 1" : "=r" (regs->pmcntenset));
	asm volatile("mrc p15, 0, %0, c9, c12, 3" : "=r" (regs->pmovsr));
	asm volatile("mrc p15, 0, %0, c9, c14, 1" : "=r" (regs->pmintenset));
	asm volatile("mrc p15, 0, %0, c9, c12, 5" : "=r" (regs->pmselr));
	asm volatile("mrc p15, 0, %0, c9, c13, 0" : "=r" (regs->pmccntr));
	asm volatile("mrc p15, 0, %0, c9, c14, 0" : "=r" (regs->pmuserenr));

	for (cnt = ARMV7_IDX_COUNTER0;
			cnt <= ARMV7_IDX_COUNTER_LAST(cpu_pmu); cnt++) {
		armv7_pmnc_select_counter(cnt);
		asm volatile("mrc p15, 0, %0, c9, c13, 1" : "=r" (val));
		regs->pmxevtyper[cnt] = val;
		asm volatile("mrc p15, 0, %0, c9, c13, 2" : "=r" (val));
		regs->pmxevcntr[cnt] = val;
	}
	put_cpu_var(pmu_regs);
}

static inline void armv7pmu_restore_PMUv1_overflow_bits(struct arm_pmu *cpu_pmu,
							struct pmu_regs *regs)
{
	unsigned int cnt;
	/* disable all ints and clear overflow flag */
	asm volatile("mcr p15, 0, %0, c9, c14, 2" : : "r" (0xffffffff));
	isb();
	asm volatile("mcr p15, 0, %0, c9, c12, 3" : : "r" (0xffffffff));
	isb();

	/* set counters to max to get the overflow with sw incr */
	for (cnt = ARMV7_IDX_COUNTER0;
			cnt <= ARMV7_IDX_COUNTER_LAST(cpu_pmu); cnt++) {
		if (!(regs->pmovsr & BIT(cnt-ARMV7_IDX_COUNTER0)))
			continue;
		...
		/* set counter to max */
		asm volatile("mcr p15, 0, %0, c9, c13, 2" : : "r" (0xffffffff));
		/* enable counter */
		asm volatile("mcr p15, 0, %0, c9, c12, 1" : : "r" (cnt));
	}

	/*enable all counters*/
	asm volatile("mcr p15, 0, %0, c9, c12, 0" : : "r" (0x1));
	/* increment counters that need overflow */
	asm volatile("mcr p15, 0, %0, c9, c12, 4" : : "r" (regs->pmovsr));
	/* disable all counters */
...
static void armv7pmu_restore_state(struct arm_pmu *cpu_pmu)
{
	unsigned int cnt;
	struct pmu_regs *regs;
	...
	/* put the overflow bits back to saved state */
	if ((regs->pmovsr & ARMV7_COUNTER_MASK) != 0) {
		if (pmu_ext < ID_DFR0_PMUV2) {
			armv7pmu_restore_PMUv1_overflow_bits(cpu_pmu, regs);
		} else {
			/* PMOVSR can be restored by writing PMOVSSET in PMUv2*/
			asm volatile("mcr p15, 0, %0, c9, c14, 3" : : "r" (regs->pmovsr));
		}
	}
	...
	for (cnt = ARMV7_IDX_COUNTER0;
			cnt <= ARMV7_IDX_COUNTER_LAST(cpu_pmu); cnt++) {
		armv7_pmnc_select_counter(cnt);
		asm volatile("mcr p15, 0, %0, c9, c13, 1" : : "r" (regs->pmxevtyper[cnt]));
		asm volatile("mrc p15, 0, %0, c9, c13, 2" : : "r" (regs->pmxevcntr[cnt]));
	}

	/* restore rest */
	asm volatile("mcr p15, 0, %0, c9, c12, 1" : : "r" (regs->pmcntenset));
	asm volatile("mcr p15, 0, %0, c9, c14, 1" : : "r" (regs->pmintenset));
	asm volatile("mcr p15, 0, %0, c9, c14, 0" : : "r" (regs->pmuserenr));
	asm volatile("mcr p15, 0, %0, c9, c13, 0" : : "r" (regs->pmccntr));
	asm volatile("mcr p15, 0, %0, c9, c12, 5" : : "r" (regs->pmselr));
	asm volatile("mcr p15, 0, %0, c9, c12, 0" : : "r" (regs->pmcr));
...
static void armv7pmu_init(struct arm_pmu *cpu_pmu)
{
	...
	cpu_pmu->save_state	= armv7pmu_save_state;
	cpu_pmu->restore_state	= armv7pmu_restore_state;
	cpu_pmu->max_period	= (1LLU << 32) - 1;

	asm volatile("mrc p15, 0, %0, c0, c1, 2" : "=r" (val));
	pmu_ext = (val & (0xf<<24))>>24;
};
