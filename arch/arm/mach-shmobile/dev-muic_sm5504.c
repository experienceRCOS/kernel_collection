...
static struct wake_lock sm5504_jig_wakelock;
...
//sm5504_charger_cb
static void sm5504_charger_cb(int32_t cable_type)
{
...
//sm5504_ovp_cb
static void sm5504_ovp_cb(void)
{
	pr_info("%s\n",__func__);
#ifdef CONFIG_SEC_CHARGING_FEATURE
...
//sm5504_usb_cb
static void sm5504_usb_cb(uint8_t attached)
{
#if defined(CONFIG_SEC_CHARGING_FEATURE)
	int spa_data = POWER_SUPPLY_TYPE_BATTERY;
#endif
	pr_info("sm5504_usb_cb attached %d\n", attached);
...
//sm5504_uart_cb
static void sm5504_uart_cb(uint8_t attached)
{
	pr_info("sm5504_uart_cb attached %d\n", attached);
   ...
   else
   {
	...
	if(attached)
		wake_lock(&sm5504_jig_wakelock);
	else
		wake_unlock(&sm5504_jig_wakelock);
#endif
}

//sm5504_jig_cb
static void sm5504_jig_cb(jig_type_t type, uint8_t attached)
{
	pr_info("sm5504_jig_cb attached %d %d\n", type, attached);
}


#ifdef USE_USB_UART_SWITCH
static ssize_t sm5504_show_UUS_state(struct device *dev,
...
static DEVICE_ATTR(UUS_state, S_IRUGO, sm5504_show_UUS_state, NULL);
#endif
...
static int usb_sysfs_init(void)
{
 	...
 	if (!usb_kobj)
 		return 0;
 	ret = sysfs_create_group(usb_kobj, &sm5504_group);
 	if (ret)
...
void jig_force_sleep(void)
{
	...
	if (wake_lock_active(&sm5504_jig_wakelock)) {
		wake_unlock(&sm5504_jig_wakelock);
...
static void sm5504_wakelock_init(void)
{
    wake_lock_init(&sm5504_jig_wakelock, WAKE_LOCK_SUSPEND,
    "sm5504_jig_wakelock");
}

#ifdef CONFIG_MFD_SM5504
static struct sm5504_platform_data  sm5504_pdata = {
    .irq_gpio = 97,
    .cable_chg_callback = sm5504_charger_cb,
    .ocp_callback = NULL,
    .otp_callback = NULL,
    .ovp_callback = sm5504_ovp_cb,
    .usb_callback = sm5504_usb_cb,
    .uart_callback = sm5504_uart_cb,
    .otg_callback = NULL,
    .jig_callback = sm5504_jig_cb,
};
...
/* Add for Ricktek RT8969 */
	{I2C_BOARD_INFO("sm5504", 0x28>>1),
	.platform_data = &sm5504_pdata,},
};

void __init dev_muic_sm5504_init(void)
{
	int ret;
	pr_info("sm5504: micro_usb_i2c_devices_info\n");
	sm5504_wakelock_init();
