...
static void cpg_dbg_check_integrity_values(const struct clk_rate rates)
{
	struct clk_rate *orig_rates;

	orig_rates = &__shmobile_freq_modes[cpg_dbg_clkmode];

	BUG_ON(rates.i_clk != orig_rates->i_clk);
	BUG_ON(rates.zg_clk != orig_rates->zg_clk);
	BUG_ON(rates.b_clk != orig_rates->b_clk);
	BUG_ON(rates.m1_clk != orig_rates->m1_clk);
	BUG_ON(rates.m3_clk != orig_rates->m3_clk);
	BUG_ON(rates.m5_clk != orig_rates->m5_clk);
	BUG_ON(rates.zt_clk != orig_rates->zt_clk);
	BUG_ON(rates.zx_clk != orig_rates->zx_clk);
	BUG_ON(rates.hp_clk != orig_rates->hp_clk);
	BUG_ON(rates.zs_clk != orig_rates->zs_clk);
	BUG_ON(rates.zb_clk != orig_rates->zb_clk);
	BUG_ON(rates.zb3_clk != orig_rates->zb3_clk);
}
...
static int cpg_set_freq(const struct clk_rate rates)
{
	...
	cpg_dbg_check_integrity_values(rates);
	...
	for (i = 0; i < CPG_SET_FREQ_MAX_RETRY; i++) {
		...
		if (frq_change) {
			...
			/* apply setting */
			memory_log_dump_int(0x70, (int)reg);
			__raw_writel(reg, FRQCRB);
			memory_log_dump_int(0x71, (int)__raw_readl(FRQCRB));
			...
			/* apply setting */
			memory_log_dump_int(0x72, (int)reg);
			__raw_writel(reg, FRQCRA);
			memory_log_dump_int(0x73, (int)__raw_readl(FRQCRA));

			cpg_dbg_check_div_rates();
			/* set and wait for KICK bit changed */
			ret = cpg_set_kick(KICK_WAIT_INTERVAL_US);
			memory_log_dump_int(0x74, (int)__raw_readl(FRQCRA));
			if (ret) {
				if (zs_change) {
...
int __init pm_setup_clock(void)
{
	...
	do {
		...
		value = __raw_readl(SMSTPCR5);
		value &= ~(1 << 22);
		__raw_writel(value, SMSTPCR5);
