...
#if defined(CONFIG_SEC_DEBUG_INFORM)
	sec_debug_inform_write(0x12345678, 2);
#else
	writel(0x12345678,BROADCOM_INFORM2);	/* Don't enter lpm mode */
#endif

	if (!cmd) {
#if defined(CONFIG_SEC_DEBUG_INFORM)
			sec_debug_inform_write(REBOOT_MODE_PREFIX | REBOOT_MODE_NONE, 3);
#else
		writel(REBOOT_MODE_PREFIX | REBOOT_MODE_NONE, BROADCOM_INFORM3);
#endif
	} else {
		unsigned long value;
		if (!strcmp(cmd, "fota"))
...
#if defined(CONFIG_SEC_DEBUG_INFORM)
			sec_debug_inform_write(REBOOT_MODE_PREFIX | REBOOT_MODE_RECOVERY, 3);
#else
...
#if defined(CONFIG_SEC_DEBUG_INFORM)
			sec_debug_inform_write(REBOOT_MODE_PREFIX | REBOOT_MODE_DOWNLOAD, 3);
#else
...
#if defined(CONFIG_SEC_DEBUG_INFORM)
			sec_debug_inform_write(REBOOT_MODE_PREFIX | REBOOT_MODE_UPLOAD, 3);
#else
...
#if defined(CONFIG_SEC_DEBUG_INFORM)
			sec_debug_inform_write(REBOOT_SET_PREFIX | REBOOT_SET_DEBUG | value, 3);
#else
			writel(REBOOT_SET_PREFIX | REBOOT_SET_DEBUG | value,
			 ...
			 && !kstrtoul(cmd + 5, 0, &value))
#if defined(CONFIG_SEC_DEBUG_INFORM)
			sec_debug_inform_write(REBOOT_SET_PREFIX | REBOOT_SET_SWSEL | value, 3);
#else
...
#if defined(CONFIG_SEC_DEBUG_INFORM)
			sec_debug_inform_write(REBOOT_SET_PREFIX | REBOOT_SET_SUD | value, 3);
#else
			writel(REBOOT_SET_PREFIX | REBOOT_SET_SUD | value,
			       BROADCOM_INFORM3);
#endif
		else if (!strncmp(cmd, "stm", 3)
			 && !kstrtoul(cmd + 3, 0, &value))
#if defined(CONFIG_SEC_DEBUG_INFORM)
			sec_debug_inform_write(REBOOT_SET_PREFIX | REBOOT_SET_STM | value, 3);
#else
			/* nil */
#endif
		else
