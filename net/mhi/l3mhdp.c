...
#define MHDP_USE_STATIC_ALLOCATION

#ifdef MHDP_USE_STATIC_ALLOCATION
#define MAX_PDN_ID 4
#endif /*MHDP_USE_STATIC_ALLOCATION*/
...
#ifndef MHDP_USE_STATIC_ALLOCATION
static void
...
mhdp_netdev_rx(struct sk_buff *skb, struct net_device *dev)
{
	...
	for (i = 0; i < mhdpHdr->packet_count; i++) {
		DPRINTK(" packet_info[%d] - PDNID:%d, packet_offset: %d, packet_length: %d\n",
...
void mhdp_create_static_tunnels(struct net *net)
{
	int pdn_id;
	struct mhdp_tunnel_parm k_parms;


	for (pdn_id = 0; pdn_id <= MAX_PDN_ID; pdn_id++) {

		sprintf(k_parms.name, MHDP_IFNAME, pdn_id);
		sprintf(k_parms.master, "smc0");
		k_parms.pdn_id = pdn_id + 1; /* This is indeed CID as modem sees it as CID */

		if (!mhdp_add_tunnel(net, &k_parms))
			printk(KERN_ERR "mhdp cannot add tunnel for pdnid %d\n",
					pdn_id);
	}
}
#endif /*MHDP_USE_STATIC_ALLOCATION*/
	...
	case NETDEV_REGISTER:
		{
			...
			struct net *tmp_net = dev_net(event_dev);

			if (mhdpn->static_alloc_done == 0) {
				mhdpn->static_alloc_done = 1;
				mhdp_create_static_tunnels(tmp_net);
