...
#ifdef CONFIG_PM_FORCE_SLEEP
#define fsleep_ignore_dpm_error(dev, error, dump_id)			\
{									\
	int dev_id = 0;							\
	if (dev)							\
		dev_id = kstrtol(dev_name(dev), 10, NULL);		\
	if (pm_is_force_sleep() && error) {				\
		memory_log_dump_int(dump_id, dev_id);			\
		error = 0;						\
	}								\
}
#endif
...
#ifdef CONFIG_PM_FORCE_SLEEP
		fsleep_ignore_dpm_error(dev, error,
				PM_DUMP_ID_FSLEEP_DPM_SUSP_NOIRQ);
#endif
		mutex_lock(&dpm_list_mtx);
		if (error) {
			pm_dev_err(dev, state, " noirq", error);
...
#ifdef CONFIG_PM_FORCE_SLEEP
		fsleep_ignore_dpm_error(dev, error,
				PM_DUMP_ID_FSLEEP_DPM_SUSP_LATE);
#endif
		mutex_lock(&dpm_list_mtx);
		if (error) {
...
static int __device_suspend(struct device *dev, pm_message_t state, bool async)
{
 ...
 End:
#ifdef CONFIG_PM_FORCE_SLEEP
	fsleep_ignore_dpm_error(dev, error, PM_DUMP_ID_FSLEEP_DPM_SUSP);
#endif
	if (!error) {
...
int dpm_suspend(pm_message_t state)
{
	...
	int error = 0;
#ifdef CONFIG_PM_FORCE_SLEEP
	int force_sleep = pm_is_force_sleep();
#else
	int force_sleep = 0;
#endif
	...
	if (error) {
#ifdef CONFIG_PM_FORCE_SLEEP
		fsleep_ignore_dpm_error(NULL, error,
				PM_DUMP_ID_FSLEEP_DPM_ASYNC_ERR);
#endif
