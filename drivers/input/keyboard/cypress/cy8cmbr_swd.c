...
static u8 GetChipProtectionVal(void)
{
	...
	dev_dbg(g_sd->dev, "%s: chipProtectionData_Chip=%d\n", __func__, 
...
static u8 GetTransitionMode(void)
{
	...
	dev_dbg(g_sd->dev, "%s: chipProtectionData_Chip=%s\n", __func__, 
...
#define SetXresHigh()		g_sd->power(1)
#define SetXresLow()		g_sd->power(0)
...
static u8 EraseAllFlash(void)
{
	...
	if ((flow == OPEN_XXX) || (flow == VIRGIN_OPEN))
    {
		...
		dev_dbg(g_sd->dev, "%s: erase all succeeded\n", __func__);
    }
    ...
    else if (flow == PROT_XXX)
    {
        ...
        dev_dbg(g_sd->dev, "%s: move to open state succeeded\n", __func__);
    }
...
static u8 ProgramFlash(void)
{
	...
	#endif
		if (rowCount == 0)
			dev_dbg(g_sd->dev, "%s: row%d file=0x%02x 0x%02x 0x%02x 0x%02x\n", __func__,
...
{ 
	...
	for ( rowCount = 0; rowCount < numOfFlashRows; rowCount++)
	{
		...
		if (rowCount == 0)
			dev_dbg(g_sd->dev, "%s: row%d chip=0x%02x 0x%02x 0x%02x 0x%02x\n", __func__,
...
static u8 VerifyProtectionSettings(void)
{
	...
	for (i = 0; i < rowProtectionByteSize; i += 4)
	{
		...
		if (i == 0)
		    dev_dbg(g_sd->dev, "%s: rowProtectionFlashData0~2:0x%02x 0x%02x 0x%02x \n", __func__,
				rowProtectionData[0], rowProtectionData[1], rowProtectionData[2]);
	}
	...
	size_t  fw_img_size)
{
	...
	dev_dbg(dev, "%s: swdio_gpio=%d\n", __func__, swdio_gpio);
	dev_dbg(dev, "%s: swdck_gpio=%d\n", __func__, swdck_gpio);
	dev_dbg(dev, "%s: fw_img_size=0x%04x\n", __func__, fw_img_size);
	dev_dbg(dev, "%s: fw_img=%p\n", __func__, fw_img);
	dev_dbg(dev, "%s: fw_img[0x00, 0x01, 0x%04x, 0x%04x]=0x%02x 0x%02x 0x%02x 0x%02x\n", 
		__func__, 
