...
int ist30xx_init_touch_driver(struct ist30xx_data *data)
{
	...
	if (ret) {
		pr_err("%s:ist30xx_cmd_run_device failed\n",__func__);
		goto error;
	}

	ret = ist30xx_get_ver_info(data);
	if (ret) {
		pr_err("%s:ist30xx_get_ver_info failed\n",__func__);
		goto error;
	}

	ret = ist30xx_start(data);
	if (ret) {
		pr_err("%s:ist30xx_start failed\n",__func__);
		goto error;
	}
...
static const struct of_device_id ist30xx_of_match[] = {
	{.compatible = "imagis,ist30xx",},
	{},
};
MODULE_DEVICE_TABLE(of, ist30xx_of_match);
				   ...
				   const struct i2c_device_id * id)
{
	...
	ret = devm_request_threaded_irq(&client->dev,
			client->irq, NULL, ist30xx_irq_thread,
	...
	if (ret) {
		ist30xx_disable_irq(data);
		dev_err(&client->dev, "DTS:ist30xx_init_touch_driver failed\n");
		goto err_irq_dts;
	}
...
# endif
	...
	return 0;
err_irq_dts:
	pr_err("[ TSP ] Error, ist30xx init touch driver\n");
	if (client->irq)
...
static struct i2c_driver ist30xx_i2c_driver = {
	...
	.driver		= {
		...
		.of_match_table = ist30xx_of_match,
#ifdef CONFIG_HAS_EARLYSUSPEND
		.pm	= &ist30xx_pm_ops,
#endif
	},
