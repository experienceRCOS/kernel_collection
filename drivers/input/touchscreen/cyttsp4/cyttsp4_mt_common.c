...
#if VERBOSE_TOUCH_LOG_WITH_LONG_PRESS
#define LONG_PRESS() ( jiffies_to_msecs(jiffies - md->jiffies) > \
	(VERBOSE_TOUCH_LOG_WITH_LONG_PRESS * 1000) )
#endif

#define LOG_TMA1036_GT\
...
static inline void cyttsp4_touch_log(struct cyttsp4_mt_data *md,
		struct cyttsp4_touch *tch, int t)
{
	struct device *dev = &md->ttsp->dev;
	struct cyttsp4_sysinfo *si = md->si;
#if VERBOSE_TOUCH_LOG_WITH_LONG_PRESS
	if (LONG_PRESS()) {
		if (si->si_ofs.tch_rec_size > CY_TMA1036_TCH_REC_SIZE)
			dev_info(dev, LOG_TMA1036_GT);
		else
			dev_info(dev, LOG_TMA1036_LE);
		return;
	}
#endif
	if (si->si_ofs.tch_rec_size > CY_TMA1036_TCH_REC_SIZE)
		dev_dbg(dev, LOG_TMA1036_GT);
	else
		dev_dbg(dev, LOG_TMA1036_LE);
}
		...
		if (sig != CY_IGNORE_VALUE) {
			...
			if (t > 2)
			{
				...
				if (tch.abs[CY_TCH_E] == CY_EV_TOUCHDOWN)
					printk("%s: tcn=[%d] id=%d e=%d PRESS x=%d, y=%d \n",__func__,
					    num_cur_tch,  t, tch.abs[CY_TCH_E], tch.abs[CY_TCH_X], tch.abs[CY_TCH_Y]);
				else
				if (tch.abs[CY_TCH_E] == CY_EV_LIFTOFF)
					printk("%s: tcn=[%d] id=%d e=%d RELEASE x=%d, y=%d \n",__func__,
					    num_cur_tch,  t, tch.abs[CY_TCH_E], tch.abs[CY_TCH_X], tch.abs[CY_TCH_Y]);
			#endif
			...
			if(num_cur_tch==1){
				...
				if(num_cur_tch==1){
					cyttsp4_set_touch_boost_mode( BOOST_DISABLE );
				}
#endif
#if !defined(CONFIG_SAMSUNG_PRODUCT_SHIP)
				printk("%s: tcn=[%d] id=%d e=%d RELEASE x=%d, y=%d \n",__func__,
...
#endif
#if defined(TOUCH_BOOSTER_SHMOBILE)
			cyttsp4_set_touch_boost_mode( BOOST_ENABLE );
#endif
			}
