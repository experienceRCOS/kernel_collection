...
static DEVICE_ATTR(tsp_threshold, S_IRUGO,
	threshold_show, NULL);/* threshold return, store */
...
static DEVICE_ATTR(brightness, S_IWUSR | S_IWGRP, NULL,  touch_led_control);
#endif
#ifdef SEC_TSP_FACTORY_TEST
...
static void cyttsp4_ts_early_suspend(struct early_suspend *h)
{
	...
	gpio_pull_down_port(CYTTSP4_ENABLE_GPIO);	
}
static void cyttsp4_ts_late_resume(struct early_suspend *h)
{
	int rc;
...
static int cyttsp4_core_probe(struct cyttsp4_core *core)
{
	...
	sec_touchscreen = device_create(sec_class, NULL, 0, cd, "sec_touchscreen");
