		...
		void (*cont) (const struct firmware *fw, void *context))
{
	...
	retval = request_firmware_nowait(THIS_MODULE,
		FW_ACTION_NOHOTPLUG, CY_FW_FILE_NAME, &ttsp->dev,
...
static int cyttsp4_fw_upgrade_platform(struct cyttsp4_device *ttsp, bool forced)
{
	...
	dev_info(dev, "%s: fw ver D : 0x%04x, fw ver B : 0x%04x\n", __func__,
			fws[CYTTSP4_SILICON_REV_D].fw_version,
			fws[CYTTSP4_SILICON_REV_B].fw_version);
...
static void __init cyttsp4_loader_init_async(void *unused, async_cookie_t cookie)
{
	int rc = 0;
	int i, j;

	{
		extern int cyttsp4_i2c_init(void);
		cyttsp4_i2c_init();
	}
