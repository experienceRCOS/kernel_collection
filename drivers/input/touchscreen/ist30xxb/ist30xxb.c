...
#if TOUCH_BOOSTER_SHMOBILE
static struct delayed_work delayed_cpufreq_start_work;
static struct delayed_work delayed_cpufreq_stop_work;
...
static void delayed_stop_cpufreq(struct work_struct *work)
{
	if (dfs_started) 
{
			int ret = 0;
...
static void setting_cpufreq(uint32_t on)
{
	if ( 0 == on ){
		if (!dfs_started)
			schedule_delayed_work(&delayed_cpufreq_start_work, msecs_to_jiffies(1000));
	}else if ( 1 == on ){
		cancel_delayed_work_sync(&delayed_cpufreq_start_work);
		if (dfs_started) {
			schedule_delayed_work(&delayed_cpufreq_stop_work, 0);
		}
	}
	else
				   ...
				   const struct i2c_device_id * id)
{
	...
	INIT_DELAYED_WORK(&delayed_cpufreq_start_work, delayed_start_cpufreq);
	INIT_DELAYED_WORK(&delayed_cpufreq_stop_work, delayed_stop_cpufreq);
#endif
