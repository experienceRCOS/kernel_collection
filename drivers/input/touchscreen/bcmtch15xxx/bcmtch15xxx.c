...
/* -------------------------------------------------------------------
	Define KEY_LED_CONTROL_FRAMEWORK to 1 in case of android framework
	supporting touch led on/off functionality through Sysfs. Undefined
	it will enable the touch key led ON/OFF functionality
	through touch driver.
	------------------------------------------------------------------ */
/*#define KEY_LED_CONTROL_FRAMEWORK 1 */
#ifndef KEY_LED_CONTROL_FRAMEWORK
static struct workqueue_struct *workqueue;

struct bcmtch_psdata {
	struct work_struct ps_work_poll;
	struct hrtimer ps_timer;
	u16  ps_poll_interval;
};
static struct bcmtch_psdata *bcmtch_ginfo;
struct bcmtch_data *bcmtch_data1;

static void bcmtch_settouchkeyledoff(void)
{
	struct bcmtch_data *bcmtch_data_ptr = bcmtch_data1;

	if (bcmtch_data_ptr->regulator_avdd33 == NULL) {
		pr_err("[TouchKey] regulator_avdd33 Not initialised\n");
		return;
	}
	if (key_value == 1) {
		key_value = 0;
		regulator_disable(bcmtch_data_ptr->regulator_avdd33);
		pr_debug("bcmtch_settouchkeyledoff\n");
	} else {
		pr_debug("Touch Key allready off\n");
	}
	return;
}
static enum hrtimer_restart bcmtch_timer_func(struct hrtimer *timer)
{
	pr_debug("bcmtch_timer_func\n");
	queue_work(workqueue, &bcmtch_ginfo->ps_work_poll);
	return HRTIMER_NORESTART;
}

#endif

#ifdef KEY_LED_CONTROL_FRAMEWORK
...
static void touch_led_on()
{
	...
	if (key_value == 0) {
		key_value = 1;
		regulator_enable(bcmtch_data_ptr->regulator_avdd33);
		...
		hrtimer_start(
			&bcmtch_ginfo->ps_timer,
			ktime_set(bcmtch_ginfo->ps_poll_interval, 0),
			HRTIMER_MODE_REL
		);
		pr_debug("Touch key led turn on\n");
	} else {
			pr_debug("Touch key led ALLREADY on\n");
	}
	return;
}
#endif
...
#ifndef KEY_LED_CONTROL_FRAMEWORK
	/* Cancel HR timer */
	ret = hrtimer_cancel(&bcmtch_ginfo->ps_timer);
	pr_debug("hrtimer_cancel %d\n", ret);
	bcmtch_settouchkeyledoff();
#endif
	/* disable interrupts */
					...
					const struct i2c_device_id *id)
{
	...
	*/
...
#ifndef KEY_LED_CONTROL_FRAMEWORK
	bcmtch_data1 = bcmtch_data_ptr;

	/* allocate driver_data */
	bcmtch_ginfo = kzalloc(sizeof(struct bcmtch_psdata), GFP_KERNEL);
	if (NULL == bcmtch_ginfo) {
		pr_err("bcmtch_ginfo kzalloc error\n");
		ret_val = -ENOMEM;
		goto psdata_alloc_err;
	}
	/* work queue settings */
	workqueue = create_singlethread_workqueue("bcmtchxxx_tchkey_bklight");
	if (NULL == workqueue) {
	...
	INIT_WORK(&bcmtch_ginfo->ps_work_poll, bcmtch_settouchkeyledoff);

	hrtimer_init(&bcmtch_ginfo->ps_timer, CLOCK_MONOTONIC,
		HRTIMER_MODE_REL);


	bcmtch_ginfo->ps_timer.function = bcmtch_timer_func;
	bcmtch_ginfo->ps_poll_interval = 2;
