...
#define TS_MAX_Z_TOUCH			255
#define TS_MAX_W_TOUCH		100
#define TS_MAX_X_COORD		(480-1)
#define TS_MAX_Y_COORD		(800-1)
#endif
...
static void change_dvfs_lock(struct work_struct *work)
{
	...
	if (ret < 0)
		printk(KERN_EMERG "%s: 1booster stop failed(%d)\n",\
					__func__, __LINE__);
	else
		printk(KERN_EMERG "[TSP] %s", __func__);
}
...
static void set_dvfs_lock(struct melfas_ts_data *ts, uint32_t on)
{
	...
	if (on == 0) {
		...
		if (!ts->dvfs_lock_status) {
			ret = set_freq_limit(DVFS_TOUCH_ID, 1008000);
			if (ret < 0)
				printk(KERN_EMERG "%s: cpu lock failed(%d)\n",\
							__func__, ret);
...
{
	u8 write_buffer[3];
	printk(KERN_EMERG"[TSP] %s : TA is %s. \n", __func__, ta_status ? "on" : "off");
	if (tsp_enabled == false) {
		printk(KERN_EMERG"[TSP] tsp_enabled is 0\n");
		return;
	}
	write_buffer[0] = 0xB0;
	write_buffer[1] = 0x11;
...
{
	printk(KERN_EMERG "[TSP] TSP_boost %s\n", is_boost ? "ON" : "Off");
	if (onoff) {
...
static void melfas_ts_get_data(struct work_struct *work)
{
	...
	if (tsp_enabled == false) 
	{
		printk(KERN_EMERG "[TSP ]%s. tsp_disabled.\n", __func__);
		msleep(500);
		return;
	}
#if DEBUG_PRINT
	...
	if (read_num > 0)
	{
		...
		if (buf[0] == 0x0E) {
			printk(KERN_EMERG"[TSP] noise mode enter!!\n");
			ts->noise_mode = 1 ;
		...
		{
			if (buf[i] == 0x0F) {
				printk(KERN_EMERG "%s : ESD-DETECTED!!!\n", __func__);
				line = __LINE__;
				goto tsp_error;
			}
			...
			if (touchType == TOUCH_SCREEN)
			{
				...
				printk(KERN_EMERG "[TSP] keyID : %d, touchstate : %d\n"
					, keyID, touchState);
				if (keyID == 0x1) {
					if (touchState)
						menu_pressed = 1;
	...
	return ;
tsp_error:
	printk(KERN_EMERG "[TSP] %s: i2c failed(%d)\n", __func__, line);
	TSP_reboot();
}
...
int melfas_fw_i2c_write(char *buf, int length)
{
	...
	printk(KERN_EMERG "<MELFAS> mass erase start write ret%d\n\n", ret);
	if (ret > 0)
		return 1;
	else
		return 0;
...
static ssize_t set_tsp_firm_version_read_show(struct device *dev, struct device_attribute *attr, char *buf)
{
	...
	if (ret < 0) {
		printk(KERN_EMERG "%s : i2c_master_send [%d]\n", __func__, ret);
	}
	ret = i2c_master_recv(ts->client, (char*)&buff, 7);
	if (ret < 0) {
		printk(KERN_EMERG "%s : i2c_master_recv [%d]\n", __func__, ret);
	}
...
ssize_t set_tsp_for_inputmethod_store(struct device *dev, struct device_attribute *attr, const char *buf, size_t size)
{
	...
	if (tsp_enabled == false) {
		printk(KERN_EMERG "[TSP ]%s. tsp_enabled is 0\n", __func__);
		return 1;
	}
	if (*buf == '1' && (!is_inputmethod)) {
		is_inputmethod = 1;
		printk(KERN_EMERG "[TSP] Set TSP inputmethod IN\n");
		/* to do */
	} else if (*buf == '0' && (is_inputmethod)) {
		is_inputmethod = 0;
		printk(KERN_EMERG "[TSP] Set TSP inputmethod OUT\n");
		/* to do */
	}
	return 1;
}
...
ssize_t set_tsp_for_boost_store(struct device *dev, struct device_attribute *attr, const char *buf, size_t size)
{
	...
	printk(KERN_EMERG "[TSP] set_tsp_for_boost_store() called. %s!\n", is_boost ? "On" : "Off");
...
static int check_debug_data(struct melfas_ts_data *ts)
{
	...
	printk(KERN_EMERG "[TSP] %s disable IRQ( %d)\n", __func__, __LINE__);
	...
	printk(KERN_EMERG "[TSP] CmABS_data\n");
	for (i = 0; i < exciting_ch * sensing_ch; i++) {
		if (0 == i % exciting_ch)
			printk(KERN_EMERG "\n");
		printk(KERN_EMERG "%4d, ", CmABS_data[i]);
	}
	printk(KERN_EMERG "\n");
	/* Read Reference Data */
	setLowLevelData[0] = 0xA0;
	...
	printk(KERN_EMERG "[TSP] %s enable IRQ( %d)\n", __func__, __LINE__);
...
static void check_delta_data(struct melfas_ts_data *ts)
{
	...
	printk(KERN_EMERG "[TSP] CmDelta_data\n");
	for (i = 0; i < exciting_ch * sensing_ch; i++) {
		if (0 == i % exciting_ch)
			printk(KERN_EMERG "\n");
		printk(KERN_EMERG "%4d, ", CmDelta_data[i]);
	}
	printk(KERN_EMERG "\n");
	/* Read Reference Data */
	setLowLevelData[0] = 0xA0;
...
static int atoi(char *str)
{
	...
	if (str == NULL)
		return result;
	while (str[count] != 0/*NULL*/ && str[count] >= '0' && str[count] <= '9') {
...
{
	printk(KERN_EMERG "disp_all_deltadata_show : value %d\n", CmDelta_data[index]);
	return snprintf (buf, sizeof(buf), "%u\n",  CmDelta_data[index]);
}
...
{
	index = atoi((char*)buf);
	printk(KERN_EMERG "Delta data %d", index);
	return size;
}
...
/*
	...
	ret = melfas_i2c_write(ts->client, setLowLevelData, 6);
#if 1
	printk(KERN_EMERG "[TSP] lntensity_data\n");
	for (i = 0; i < exciting_ch * sensing_ch; i++) {
		if (0 == i % exciting_ch)
			printk(KERN_EMERG"\n");
		printk(KERN_EMERG"%4d, ", lntensity_data[i]);
	}
	printk(KERN_EMERG"\n");
#endif
...
static void release_all_fingers(struct melfas_ts_data *ts)
{
	...
	#if TOUCH_BOOSTER
	set_dvfs_lock(ts, 2);
	printk(KERN_EMERG "[TSP] dvfs_lock free.\n ");
	#endif
}
static void TSP_reboot(void)
{
...
void TSP_force_released(void)
{
	printk(KERN_EMERG "%s satrt!\n", __func__);
	if (tsp_enabled == false) {
		printk(KERN_EMERG "[TSP] Disabled\n");
		return;
	}
	release_all_fingers(ts);
	touch_is_pressed = 0;
...
int tsp_i2c_read_melfas(u8 reg, unsigned char *rbuf, int buf_size)
{
	...
	for (i = 0; i < retry; i++) {
		...
		if (i == (retry - 1)) {
			printk(KERN_EMERG "[TSP] Error code : %d, %d\n", __LINE__, ret);
		}
	}
	return ret;
}
...
static int melfas_i2c_read(struct i2c_client *p_client, u8 reg, u8 *data, int len)
{
	...
	if (1 != i2c_transfer(p_client->adapter, &msg, 1)) {
		printk(KERN_EMERG "[TSP][MMS128][%s] set data pointer fail! reg(%x)\n", __func__, reg);
		return -EIO;
	}
	...
	msg.buf = data;
	if (1 != i2c_transfer(p_client->adapter, &msg, 1)) {
		printk(KERN_EMERG "[TSP][MMS128][%s] fail! reg(%x)\n", __func__, reg);
		return -EIO;
	}
	return 0;
}
			...
			struct device_attribute *attr, char *buf)
{
	...
	printk(KERN_EMERG "firmware_panel_show : [%d]\n", panel);
...
static ssize_t firmware_show(struct device *dev, struct device_attribute *attr, char *buf)
{
	...
	if (0 == melfas_i2c_read(ts->client, MIP_TSP_REVISION, 6, buf1))
	{
		...
		printk(KERN_EMERG "[TSP][MMS128][%s]  phone_ver=%d, fw_ver=%d, hw_rev=%d\n",
			buf, phone_ver, fw_ver, hw_rev);
	} else {
		printk(KERN_EMERG "[TSP][MMS128][%s] Can't find HW Ver, FW ver!\n",
				__func__);
	}
...
{
	int ret;
	printk(KERN_EMERG "START firmware store\n");
	ts_power_enable(0);
	msleep(500);
	ts_power_enable(1);
	msleep(500);
	...
	if (ret == MCSDL_RET_SUCCESS)
		...
		firmware_ret_val = 0;
#endif
	printk(KERN_EMERG"[TSP] Firmware update end!!\n");
	ts_power_enable(0);
	msleep(500);
	ts_power_enable(1);
	printk(KERN_EMERG "firmware store END\n");
	return 0;
}
...
static ssize_t	check_init_lowleveldata( void )
{
	...
	int ret = 1;
		...
		if (ret < 0) {
			printk(KERN_EMERG "[TSP]Exciting CH. melfas_i2c_read fail! %s : %d, \n",
				__func__, __LINE__);
			return 0;
		}
		...
		if (ret < 0) {
			printk(KERN_EMERG "[TSP]Sensing CH. melfas_i2c_read fail! %s : %d, \n",
				__func__, __LINE__);
			return 0;
		}
...
struct device_attribute *attr, const char *buf, size_t size)
{
	...
	printk(KERN_EMERG "[TSP] %s, %d, value=%ld\n", __func__, __LINE__, value);
	return size;
}
static ssize_t tkey_rawcounter_show(struct device *dev, \
			struct device_attribute *attr, char *buf)
{
	printk(KERN_EMERG "[TSP] menukey : %d, backKey : %d\n", \
	...
	struct device_attribute *attr, char *buf)
{
	TSP_reboot();
	msleep(300);
	printk(KERN_EMERG "set_tsp_test_mode_disable0 \n");
	tsp_testmode = 0;
	return 0;
}
...
{
	tsp_testmode = 1;
	printk(KERN_EMERG "set_tsp_test_mode_enable0 \n");
	mdelay(50);
	ts_power_enable(0);
	mdelay(500);
	ts_power_enable(1);
...
{
	index = atoi((char*)buf);
	printk(KERN_EMERG "Intensity data %d", index);
	return size;
}
ssize_t disp_all_intendata_show(struct device *dev,
	struct device_attribute *attr, char *buf)
{
	printk(KERN_EMERG "disp_all_intendata_show : value %d, index=%d\n",
		...
		struct device_attribute *attr, char *buf)
{
	...
	for (i = 0; i < sensing_ch; i++) {
		...
		if (ret < 0)
			printk(KERN_EMERG "can't read rawdata_pass_fail_tst200 Data %dth\n", i);
		udelay(5);
		...
		for (j = 0; j < read_data_len; j++) {
			printk(KERN_EMERG "[%03d],", read_data_buf[j]);
		}
		printk(KERN_EMERG "\n");
#endif
		msleep(1);
	...
	struct device_attribute *attr, char *buf)
{
	...
	for (i = 0; i < DOWNLOAD_RETRY_CNT; i++) {
		ret = MFS_ISC_update();
		printk(KERN_EMERG "mcsdl_download_binary_data : [%d]\n", ret);
		if (ret != 0)
			printk(KERN_EMERG "SET Download Fail - error code [%d]\n",
				ret);
		else
			break;
	}
...
static void fw_update(void *device_data)
{
	...
	for (i = 0; i < DOWNLOAD_RETRY_CNT; i++) {
		ret = MFS_ISC_update();
		printk(KERN_EMERG "mcsdl_download_binary_data by kyestring : [%d]\n",
			ret);
		if (ret != 0) {
			printk(KERN_EMERG \
...
static void check_delta_value(struct melfas_ts_data *ts)
{
	...
	printk(KERN_EMERG "min:%d,max:%d", min_value, max_value);
	printk(KERN_EMERG "\n");
	snprintf(buff, sizeof(buff), "%d,%d", min_value, max_value);
	set_cmd_result(ts, buff, strnlen(buff, sizeof(buff)));
	/* Read Reference Data */
...
init_again:
	...
	if (!i2c_check_functionality(client->adapter, I2C_FUNC_I2C)) {
		printk(KERN_EMERG "%s: need I2C_FUNC_I2C\n", __func__);
		ret = -ENODEV;
		goto err_check_functionality_failed;
	}
	ts = kmalloc(sizeof(struct melfas_ts_data), GFP_KERNEL);
	if (ts == NULL) {
		printk(KERN_EMERG "%s: failed to create a state of melfas-ts\n", __func__);
		ret = -ENOMEM;
    ...
    if (!ts->input_dev) {
		printk(KERN_EMERG "%s: Not enough memory\n", __func__);
		ret = -ENOMEM;
    ...
    if (ret) {
	printk(KERN_EMERG "%s: Failed to register device\n", __func__);
	ret = -ENOMEM;
    ...
    if (ts->client->irq) {
#if DEBUG_PRINT
	printk(KERN_EMERG "%s: trying to request irq: %s-%d\n", __func__,
...
#endif
#if defined(USE_TEST_RAW_TH_DATA_MODE) && USE_TEST_RAW_TH_DATA_MODE
...
err_input_register_device_failed:
	printk(KERN_EMERG "melfas-ts: err_input_register_device failed\n");
	input_free_device(ts->input_dev);
err_input_dev_alloc_failed:
	printk(KERN_EMERG "melfas-ts: err_input_dev_alloc failed\n");
err_alloc_data_failed:
	printk(KERN_EMERG "melfas-ts: err_alloc_data failed_\n");
...
err_check_functionality_failed:
	printk(KERN_EMERG "melfas-ts: err_check_functionality failed_\n");
	return ret;
...
static int melfas_ts_resume(struct i2c_client *client)
{
	...
	mms_set_noise_mode(ts);
	tsp_enabled = true;
	printk(KERN_EMERG "[TSP] %s enable IRQ( %d) FW:0x%x\n", __func__, __LINE__,TSP_PanelVersion);
