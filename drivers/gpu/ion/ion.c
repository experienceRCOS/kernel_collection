				     ...
				     unsigned long flags)
{
	...
	if (r_mobile_ion_map_pnc_nma(buffer)) {
...
static void _ion_buffer_destroy(struct kref *kref)
{
	...
	mutex_lock(&dev->buffer_lock);
#ifdef CONFIG_ION_R_MOBILE
	r_mobile_ion_unmap_pnc_nma(buffer);
#endif
...
static int ion_handle_add(struct ion_client *client, struct ion_handle *handle)
{
	...
	do {
		...
		rc = idr_alloc(&client->idr, handle, 0, 0, GFP_NOWAIT);
		idr_preload_end();
		handle->id = rc;
	} while (rc == -EAGAIN);
...
#ifdef CONFIG_ION_R_MOBILE
struct ion_buffer *ion_buffer_lock(struct ion_client *client,
		struct ion_handle *handle)
{
	struct ion_buffer *buffer = NULL;
	...
	mutex_lock(&client->lock);
	if (!ion_handle_validate(client, handle)) {
		pr_err("Invalid handle passed to lock the buffer.\n");
		mutex_unlock(&client->lock);
		return NULL;
	}
	buffer = handle->buffer;
...
void ion_buffer_unlock(struct ion_client *client, struct ion_buffer *buffer)
{
	mutex_unlock(&buffer->lock);
	mutex_unlock(&client->lock);
}
