...
int ion_get_total_mem_bypid(pid_t pid)
{
	struct ion_client *client_tmp;
	struct rb_node *n;
	size_t sizes[ION_NUM_HEAP_IDS] = {0};
	int i,total_size=0;
	pr_debug("ion_get_total_mem_bypid is called\n");
	...
	for (n = rb_first(&client_tmp->handles); n; n = rb_next(n)) {
		struct ion_handle *handle = rb_entry(n, struct ion_handle,
						     node);
		unsigned int id = handle->buffer->heap->id;
		sizes[id] += handle->buffer->size;
	...
	pr_debug("ion_get_total_mem_bypid count with pid:%d, ion : %d \n",pid, total_size);
	return total_size;
}

EXPORT_SYMBOL(ion_get_total_mem_bypid);
#endif /* #ifdef CONFIG_KEEP_GRAPHIC_BOUNDED_PROCESS */
#endif
