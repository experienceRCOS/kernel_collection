...
int sec_nfc_i2c_probe(struct i2c_client *client)
{
	...
	wake_lock_init(&info->nfc_wake_lock,
...
#endif
static int sec_nfc_probe_do(struct device *dev)
{
...
static int sec_nfc_remove_do(struct device *dev)
{
	struct sec_nfc_info *info = dev_get_drvdata(dev);
	struct sec_nfc_platform_data *pdata = info->pdata;
		...
		const struct i2c_device_id *id)
{
	...
	ret = sec_nfc_probe_do(&client->dev);
	if (ret)
		return ret;

	if (sec_nfc_i2c_probe(client))
		sec_nfc_remove_do(&client->dev);
...
static int sec_nfc_remove(struct i2c_client *client)
{
	...
	return sec_nfc_remove_do(&client->dev);
}
