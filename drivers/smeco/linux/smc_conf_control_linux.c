...
static void smc_conf_update(phys_addr_t start);
...
static void smc_conf_update(phys_addr_t shm_start_address)
	...
	smc_cpu_name = smc_instance_conf_name_get_from_list(
		smc_instance_conf_control, SMC_CONF_COUNT_CONTROL,
		SMC_CONFIG_USER_CONTROL, TRUE, 2, 0);
	smc_instance_conf = smc_instance_conf_get_control(
		SMC_CONFIG_USER_CONTROL, smc_cpu_name);
	smc_instance_conf->shm_start_address = shm_start_address +
		SMC_CONF_CONTROL_SHM_START_OFFSET_ES20;
	}
/*
