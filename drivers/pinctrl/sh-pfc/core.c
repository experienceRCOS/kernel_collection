...
{
	if (pfc->info->ranges == NULL)
		pfc->nr_pins = pfc->info->nr_pins;
	else
		pfc->nr_pins = pfc->info->ranges[pfc->info->nr_ranges-1].end + 1;
...
static int sh_pfc_probe(struct platform_device *pdev)
{
	...
	/*
	 * Initialize the GPIO chip first - necessary if pinctrl
	 * hogs want to use the GPIO-driving pinconfs.
	 */
	ret = sh_pfc_register_gpiochip(pfc);
	if (unlikely(ret != 0)) {
		/*
		 * If the GPIO chip fails to come up we still proceed to
		 * register with pinctrl.
		 */
	...
	/*
	 * Then register with the pinctrl core
	 */
...
#ifdef CONFIG_GPIO_SH_PFC
	/*
	 * Finally set up the GPIO<->pin mappings
	 */
	ret = sh_pfc_register_gpio_pin_mappings(pfc);
	if (unlikely(ret != 0)) {
		dev_notice(pfc->dev, "failed to map GPIO to pins, ignoring...\n");
	}
#endif
