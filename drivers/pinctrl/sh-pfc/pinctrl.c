...
struct sh_pfc_pin_config {
	unsigned type:16;	/* Current mux+config (PINMUX_TYPE_xxx) */
	bool gpio:1;		/* Claimed with gpio_request_enable */
};
				...
				unsigned offset)
{
	...
	seq_printf(s, "%s %-8s", pfc->info->name, cfg_name[cfg->type]);
	if (pfc->gpio)
		seq_printf(s, " %s",
			sh_pfc_pin_get_value(pfc, offset) ? "hi" : "lo");
	if (pfc->info->ops && pfc->info->ops->dbg_show)
		pfc->info->ops->dbg_show(pfc, s, offset);
				      ...
				      unsigned offset)
{
	...
	if (cfg->type == PINMUX_TYPE_FUNCTION) {
		dev_err(pfc->dev,
				    ...
				    enum pin_config_param param)
{
	...
	case PIN_CONFIG_OUTPUT:
		return pfc->gpio && (pin->configs & SH_PFC_PIN_CFG_OUTPUT);

	case PIN_CONFIG_BIAS_HIGH_IMPEDANCE:
		return pfc->gpio && (pin->configs & SH_PFC_PIN_CFG_INPUT);
			      ...
			      unsigned long *config)
{
	...
	switch (param) {
	case PIN_CONFIG_BIAS_HIGH_IMPEDANCE:
		if (cfg->type != PINMUX_TYPE_INPUT)
			return -EINVAL;

		/* If pulling, we're not high impedance */
	...
	case PIN_CONFIG_OUTPUT:
		if (cfg->type != PINMUX_TYPE_OUTPUT)
			return -EINVAL;

		*config = pinconf_to_config_packed(param,
				sh_pfc_pin_get_value(pfc, _pin));
		break;
			      ...
			      unsigned long config)
{
	...
	case PIN_CONFIG_BIAS_HIGH_IMPEDANCE:
	case PIN_CONFIG_OUTPUT:
		if (cfg->type == PINMUX_TYPE_FUNCTION || cfg->gpio)
			return -ENOTSUPP;

		spin_lock_irqsave(&pfc->lock, flags);
		if (param == PIN_CONFIG_OUTPUT) {
			sh_pfc_pin_set_value(pfc, _pin, argument);
			type = PINMUX_TYPE_OUTPUT;
		} else {
			type = PINMUX_TYPE_INPUT;
		}
		ret = sh_pfc_config_mux(pfc, pin->enum_id, type);
		if (ret == 0) {
			cfg->type = type;
			if (pfc->info->ops && pfc->info->ops->set_bias)
				pfc->info->ops->set_bias(pfc, _pin,
	...
	case PIN_CONFIG_BIAS_DISABLE:
		...
		/*
		 * When the pin is free, a pull implicitly selects input,
		 * otherwise we leave this to the function or GPIO API.
		 */
		if (cfg->type != PINMUX_TYPE_FUNCTION && !cfg->gpio) {
			ret = sh_pfc_config_mux(pfc, pin->enum_id,
						PINMUX_TYPE_INPUT);
			if (ret == 0)
				cfg->type = PINMUX_TYPE_INPUT;
		} else {
			ret = 0;
		}
		spin_unlock_irqrestore(&pfc->lock, flags);
