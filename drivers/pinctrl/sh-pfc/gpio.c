...
/* Export basic gpio set/get as "core" API for gpio-based pinconf */
void sh_pfc_pin_set_value(struct sh_pfc *pfc, unsigned pin,
			  int value)
{
	return gpio_pin_set_value(pfc->gpio, pin, value);
}

int sh_pfc_pin_get_value(struct sh_pfc *pfc, unsigned pin)
{
	return gpio_pin_get(&pfc->gpio->gpio_chip, pin);
}
...
int sh_pfc_register_gpio_pin_mappings(struct sh_pfc *pfc)
{
	...
	for (i = 0; i < nr_ranges; ++i) {
		...
		ret = gpiochip_add_pin_range(&pfc->gpio->gpio_chip,
					     dev_name(pfc->dev),
