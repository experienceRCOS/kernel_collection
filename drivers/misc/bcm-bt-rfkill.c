...
static int bcm_bt_rfkill_set_power(void *data, bool blocked)
{
	...
	pr_info("bcm_bt_rfkill_setpower: called %s %d\n",
			gpio_get_value(vreg_gpio) ? "High [POWER ON]" :
			"Low [POWER_OFF]", blocked);
...
#else
	rc = gpio_request(pdata->bcm_bt_rfkill_vreg_gpio,
				"bcm_bt_rfkill_vreg_gpio");
#endif
...
#else
        	rc = gpio_request(pdata->bcm_bt_rfkill_n_reset_gpio, 
					"bcm_bt_rfkill_n_reset_gpio");
#endif
