...
)
{
	int				   ret;
	unsigned char __iomem		   *p_cmd_addr;
	unsigned int  __iomem  *p_cmd_addr_word;
	unsigned long	   data_size;
	...
	if (SMAP_OK == ret) {
		...
		if (NULL == p_completion) {
			...
			if (SMAP_OK != ret) {
				MSG_ERROR(
			"[ICCOMK]ERR| iccom_put_recv_queue() Error(%d).\n",
						(unsigned int)ret);
		...
		MSG_ERROR("[ICCOMK]ERR| Fatal on RT side\n");

		p_cmd_addr_word =
			(unsigned int __iomem *)((g_iccom_command_area+
							ICCOM_CMD_AREA_Y1)+
		((cmd_posi >> ICCOMEICR_BIT_SHIFT_CMD) * ICCOM_CMD_AREA_SIZE));

		MSG_ERROR("----- [Return][MFI Dump] Start  -----\n");
		for (i = 0; i < 8; i++) {
			MSG_ERROR("...[MFI data] [0x%08x]\n",
						*(p_cmd_addr_word++));
		}
		MSG_ERROR("----- [Return][MFI Dump] End  -----\n");

		p_cmd_addr_word =
			(unsigned int __iomem *)(g_iccom_command_area+
							ICCOM_CMD_AREA_X1);
		MSG_ERROR("----- [Forward-1][MFI Dump] Start  -----\n");
		for (i = 0; i < 16; i++) {
			MSG_ERROR("...[MFI data] [0x%08x]\n",
						*(p_cmd_addr_word++));
		}
		MSG_ERROR("----- [Forward-1][MFI Dump] End  -----\n");

		p_cmd_addr_word =
			(unsigned int __iomem *)(g_iccom_command_area+
							ICCOM_CMD_AREA_X2);

		MSG_ERROR("----- [Forward-2][MFI Dump] Start  -----\n");
		for (i = 0; i < 16; i++) {
			MSG_ERROR("...[MFI data] [0x%08x]\n",
						*(p_cmd_addr_word++));
		}
		MSG_ERROR("----- [Forward-2][MFI Dump] End  -----\n");
