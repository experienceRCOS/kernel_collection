...
#ifdef CONFIG_KEEP_GRAPHIC_BOUNDED_PROCESS
extern int ion_get_total_mem_bypid(pid_t pid);
#define LMK_ION_THRSHOLD 120*1024*1024
#define OOM_ADJ_VAL_GBP -706
#define PERSIST_ADJ_VAL	-705
#define PHON_PROCESS  "m.android.phone"
#define SYSTEMUI_PROCESS "ndroid.systemui"
#endif
/*
...
#ifdef CONFIG_KEEP_GRAPHIC_BOUNDED_PROCESS
		if(unlikely(oom_score_adj==0))  /* Foreground App*/
		{
			if(ion_get_total_mem_bypid(tsk->pid) > LMK_ION_THRSHOLD)
				oom_score_adj = OOM_ADJ_VAL_GBP;
		}
		else if((oom_score_adj==PERSIST_ADJ_VAL)&&(!strncmp(p->comm,PHON_PROCESS,15)||!strncmp(p->comm,SYSTEMUI_PROCESS,15)))
			oom_score_adj = OOM_ADJ_VAL_GBP;

		if(oom_score_adj == OOM_ADJ_VAL_GBP)
			if (oom_score_adj < min_score_adj) {
				task_unlock(p);
				continue;
			}
