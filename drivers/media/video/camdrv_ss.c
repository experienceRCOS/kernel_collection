...
static int camdrv_ss_actual_sensor_power_restart(void )
{
	int err = 0;
...
#define SENSOR_THERMAL_RETRY_COUNT 5
...
void camdrv_ss_init_func(struct v4l2_subdev *sd)
{
	...
	bool status=0;
		...
		if(sensor.checkSensorThermalStatus != NULL) 
		{
			int sensorThermalRetry_cnt=0;

			for(sensorThermalRetry_cnt=0; sensorThermalRetry_cnt < SENSOR_THERMAL_RETRY_COUNT; sensorThermalRetry_cnt++)
			{
				status=sensor.checkSensorThermalStatus(sd);
				CAM_INFO_PRINTK("%s: status=%d \n", __func__,status);
				if(status == true )
				{
					CAM_INFO_PRINTK("%s: sensor is in good state sensorThermalRetry_cnt=%d\n", __func__,sensorThermalRetry_cnt);
					break;
				}
				else
				{
					err = camdrv_ss_actual_sensor_power_restart();
					CAM_INFO_PRINTK("%s: reset sensor power err=%d \n", __func__,err);
					err = camdrv_ss_init(sd, 1);
					CAM_INFO_PRINTK("%s: reset sensor init err=%d \n", __func__,err);
					msleep(100);
				}
				
			}
			if(sensorThermalRetry_cnt == SENSOR_THERMAL_RETRY_COUNT)
			{
				CAM_INFO_PRINTK("%s: Exceeded the retry count err=%d \n", __func__,err);
				err = -1;
			}
		}
...
static int camdrv_ss_s_stream(struct v4l2_subdev *sd, int enable)
{
	...
	if (enable && (tmp_sensor_state != CAMDRV_SS_STREAMING)) {
		...
		//Compare the sensor state
		if(camera_id==CAMDRV_SS_CAM_ID_SUB)
		{
