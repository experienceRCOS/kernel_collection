...
bool camdrv_ss_db8v61m_checkSensorThermalStatus(struct v4l2_subdev *sd)
{
	struct i2c_client *client = v4l2_get_subdevdata(sd);
    unsigned char read_control_reg1value = 0,read_status_regvalue=0, read_csi_enablevalue=0,read_control_reg2value=0,read_control_reg3value=0;
	bool status=false;
	
		
	camdrv_ss_i2c_write_2_bytes(client, 0xFF, 0xA1);
    camdrv_ss_i2c_read_1_byte(client, 0x71, &read_control_reg1value);
	CAM_INFO_PRINTK("%s : read_control_reg1value  =  %d \n",__func__,read_control_reg1value);

	camdrv_ss_i2c_write_2_bytes(client, 0xFF, 0xD0);
    camdrv_ss_i2c_read_1_byte(client, 0x02, &read_status_regvalue);
	CAM_INFO_PRINTK("%s : read_status_regvalue  =  %d \n",__func__,read_status_regvalue);
	

	camdrv_ss_i2c_write_2_bytes(client, 0xFF, 0xB0);
    camdrv_ss_i2c_read_1_byte(client, 0x04, &read_csi_enablevalue);
	CAM_INFO_PRINTK("%s : read_csi_enablevalue  =  %d \n",__func__,read_csi_enablevalue);

	camdrv_ss_i2c_write_2_bytes(client, 0xFF, 0xD1);
    camdrv_ss_i2c_read_1_byte(client, 0x08, &read_control_reg2value);
	CAM_INFO_PRINTK("%s : read_csi_enablevalue  =  %d \n",__func__,read_control_reg2value);

	camdrv_ss_i2c_write_2_bytes(client, 0xFF, 0x81);
    camdrv_ss_i2c_read_1_byte(client, 0x89, &read_control_reg3value);
	CAM_INFO_PRINTK("%s : read_control_reg3value  =  %d \n",__func__,read_control_reg3value);
	

	if(	(read_control_reg1value != 0xD) || (read_status_regvalue != 0x1) || (read_csi_enablevalue != 0x1) || (read_control_reg2value != 0xC) || (read_control_reg3value != 0x2))
		status= false ;
	else
		status = true;
...
bool camdrv_ss_sensor_functions_db8v61m(struct camdrv_ss_sensor_cap *sensor)
{
	...
	sensor->checkSensorThermalStatus = camdrv_ss_db8v61m_checkSensorThermalStatus;
