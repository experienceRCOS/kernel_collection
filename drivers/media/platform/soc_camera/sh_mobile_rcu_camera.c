...
static int sh_mobile_rcu_s_ctrl(struct v4l2_ctrl *ctrl)
{
	...
	if (ctrl->val != RCU_CUSTOM_CMD_DEFAULT_VALUE) {
		...
		case V4L2_CID_SET_MMAP_PAGE:
			...
			if (sizeof(mmap_page_info)/sizeof(mmap_page_info[0]) <= 2) {
			...
			if (copy_from_user(pcdev->mmap_pages,
					...
					(unsigned long)(page_num * (unsigned long)sizeof(struct page *)))) {
					dev_err(icd->parent,
					"%s:copy_from_user error(%d)\n",
					__func__, ret);
				kfree(pcdev->mmap_pages);
	...
	struct sh_mobile_rcu_snd_cmd *snd_cmd)
{
	...
	case SH_RCU_SNDCMD_SND:
		...
		msg[0].len = (unsigned short)(snd_cmd->snd_size&0xFFFF);
	...
	case SH_RCU_SNDCMD_RCV:
		...
		msg[0].len = (unsigned short)(snd_cmd->rcv_size&0xFFFF);
