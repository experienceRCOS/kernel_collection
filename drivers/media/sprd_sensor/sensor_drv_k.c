...
int camera_antibanding_val = 0; // Default CAM_BANDFILTER_50HZ
...
static ssize_t camera_antibanding_show(struct device *dev, struct device_attribute *attr, char *buf)
{
	int count;
...
static ssize_t camera_antibanding_store(struct device *dev, struct device_attribute *attr, const char *buf, size_t size)
{
	int tmp = 0;
...
#endif
	err = device_create_file(dev_t_rear, &camera_antibanding_attr);
	if(err)
	{
		SENSOR_PRINT("Failed to create device file(%s)!\n", camera_antibanding_attr.attr.name);
		goto err_make_antibanding_value;
	}
#if defined(CONFIG_FRONT_CAMERA_SUPPORTED)
...
err_make_antibanding_value:
device_remove_file(dev_t_rear, &camera_antibanding_attr);
