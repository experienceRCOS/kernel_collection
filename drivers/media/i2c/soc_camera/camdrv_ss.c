...
static int camdrv_ss_AE_AWB_lock(struct v4l2_subdev *sd, struct v4l2_ctrl *ctrl)
{
	...
	case AE_UNLOCK_AWB_UNLOCK:
	{
		...
		if(panorama_shot==1){
			if(sensor.panorama_ae_unlock_regs==0){
							CAM_ERROR_PRINTK("%s %s : awb_unlock_regs  supported !!!\n", sensor.name, __func__);
				}
			else{
			err = camdrv_ss_i2c_set_config_register(client, sensor.panorama_ae_unlock_regs, sensor.rows_num_panorama_ae_unlock_regs, "panorama_ae_unlock_regs");
panorama_shot=0;
			}
			}
		break;
...
static int camdrv_ss_s_ctrl(struct v4l2_ctrl *ctrl)
{
	...
	{
		CAM_ERROR_PRINTK("%s: V4L2_CID_CAMERA_SAMSUNG%d\n", __func__,  ctrl->val);
	...
	if(ctrl->val==1)
	    ...
	    return 0;
	        ...
	        if (ctrl->id == V4L2_CID_SHOT_PANORAMA)//case V4L2_CID_CAMERA_SAMSUNG:
	{
		CAM_ERROR_PRINTK("%s: V4L2_CID_SHOT_PANORAMA%d\n", __func__,  ctrl->val);

	if(ctrl->val==7)
	    panorama_shot=1;
            CAM_INFO_PRINTK( "%s %s :  panorama_shot set =%d\n", sensor.name, __func__, panorama_shot);
	    return 0;
	}
