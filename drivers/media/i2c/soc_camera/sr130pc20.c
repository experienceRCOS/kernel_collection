...
int SR130PC20_power(struct device *dev, int power_on)
{
	...
	vclk_clk = clk_get(NULL, "vclk2_clk");
	if (IS_ERR(vclk_clk)) {
		dev_err(dev, "clk_get(vclk_clk) failed\n");
		return -1;
	}
	...
	if (power_on) {
		...
		/* MCLK Sub-Camera */
		iRet = clk_set_rate(vclk_clk,
			clk_round_rate(vclk_clk, 24000000));
		if (0 != iRet) {
			dev_err(dev,
				"clk_set_rate(vclk_clk) failed (ret=%d)\n",
				iRet);
		}

		iRet = clk_enable(vclk_clk);
		if (0 != iRet) {
			dev_err(dev, "clk_enable(vclk_clk) failed (ret=%d)\n",
				iRet);
		}
		mdelay(15);
