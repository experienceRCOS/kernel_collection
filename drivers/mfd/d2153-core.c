...
#define D2153_ONKEY_RETRY_DBG_MSG 
		       ...
		       struct d2153_platform_data *pdata)
{
	...
	d2153_reg_read(d2153, D2153_STATUS_B_REG, &status_b);
	if (!(status_b&1))
		pr_info("Onkey is pressed\n");
...
{
	//u8 dst;
#ifdef D2153_ONKEY_RETRY_DBG_MSG 	// Added retry code
	u8 val=0;
#endif	
	struct d2153 *d2153 = d2153_dev_info;
	...
	if (mutex_trylock(&d2153->d2153_io_mutex) == 0) {
		dlg_err("%s: Someone has a mutex. Release it by force.\n", __func__ );
		mutex_destroy(&d2153->d2153_io_mutex);
		mutex_init(&d2153->d2153_io_mutex);
		mutex_lock(&d2153->d2153_io_mutex);
	} 
...
//  20140609 Added retry code
#ifdef D2153_ONKEY_RETRY_DBG_MSG
	while(1){
		printk("%s : power-off\n", __func__);
		...
		// debugging
		d2153_reg_read(d2153, D2153_POWER_CONT_REG, &val);
		printk("%s : D2153_POWER_CONT_REG[0x%x]\n", __func__, val);

		val=0;
		d2153_reg_read(d2153, D2153_SUPPLY_REG, &val);
		printk("%s : D2153_SUPPLY_REG[0x%x]\n", __func__, val);

		val=0;
		d2153_reg_read(d2153, D2153_CONTROL_B_REG, &val);
		printk("%s : D2153_CONTROL_B_REG[0x%x]\n", __func__, val);
