 ...
 * Richtek SM5504 Multifunction Device / MUIC Driver
 *
 * Copyright (C) 2013
...
#ifdef SAMSUNG_MVRL_MUIC_SM5504
...
#define SM5504_DRV_VER "2.1.0G"
#define SM5504_IRQF_MODE IRQF_TRIGGER_FALLING
...
unsigned int musb_get_charger_type_sm5504(void)
{
	printk("%s\n",__func__);
	BUG_ON(1);
...
EXPORT_SYMBOL(musb_get_charger_type_sm5504);

void musb_vbus_changed_sm5504(int state)
{
	printk("%s state=%d\n",__func__,state);
	BUG_ON(1);
...
EXPORT_SYMBOL(musb_vbus_changed_sm5504);
int bcm_ext_bc_status_sm5504(void)
{
	printk("%s\n",__func__);
	BUG_ON(1);
...
EXPORT_SYMBOL(bcm_ext_bc_status_sm5504);
uint8_t sm5504_get_otg_status(void)
{
...
EXPORT_SYMBOL(sm5504_get_otg_status);
...
static void sm5504_irq_work(struct work_struct *work)
{
	int ret;
	sm5504_chip_t *chip = container_of(to_delayed_work(work),
					   sm5504_chip_t, irq_work);
				  ...
				  const struct i2c_device_id *id)
{
	...
	INIT_DELAYED_WORK(&chip->irq_work, sm5504_irq_work);
	...
	ret = request_irq(chip->irq, sm5504_irq_handler,
                   SM5504_IRQF_MODE | IRQF_NO_SUSPEND,
...
{
	struct sm5504_chip *chip;
	RTINFO("Richtek SM5504 driver removing...\n");
#ifdef SAMSUNG_MVRL_MUIC_SM5504
...
MODULE_DESCRIPTION("Richtek SM5504 MUIC Driver");
