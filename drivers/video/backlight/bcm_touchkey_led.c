...
extern void touchkey_backlight_control(int on);
void touchkeyled_power_ctrl(unsigned char on_off)
{
	printk("[TouchkeyLED] %s : %d\n", __func__, on_off);
	if (on_off == TOUCHKEYLED_ON) {
		touchkey_backlight_control(1);
	} else {
		touchkey_backlight_control(0);
	}
}
...
static void touchkeyled_lateresume(struct early_suspend *desc)
{
	KLDBG("[KEYLED] %s, %d\n", __func__, __LINE__);
	//touchkeyled_power_ctrl(TOUCHKEYLED_ON);
}
...
static int touchkeyled_probe(struct platform_device *pdev)
{
	...
	touchkeyled->early_suspend_desc.suspend = ktd_backlight_earlysuspend;
	touchkeyled->early_suspend_desc.resume = touchkeyled_lateresume;
