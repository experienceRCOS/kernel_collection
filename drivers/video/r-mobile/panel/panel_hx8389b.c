...
static const struct specific_cmdset initialize_cmdset[] = {
	...
	{ MIPI_DSI_DCS_LONG_WRITE, setPanelDrivingTiming,
			sizeof(setPanelDrivingTiming) },
	/*Powersetting End*/
...
static void HX8389b_panel_esd_check_work(struct work_struct *work)
{
	...
	pr_info("ESD : HX8389b_panel_esd_check_work %d\n",
			esd_check_flag);
...
retry:
	...
	/* Transmit DSI command peculiar to a panel */
	if (retry_count == HX8389b_INIT_RETRY_COUNT)
...
struct fb_panel_func r_mobile_HX8389b_panel_func(int panel)
{
