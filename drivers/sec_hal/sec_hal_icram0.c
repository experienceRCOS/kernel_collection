...
struct mem_alloc_info {
	void __iomem *virt_addr; /* the start address of the message area */
	uint8_t size;/* the size of allocation */
	uint8_t alloc; /* status of the block */
};

struct mem_msg_area {
	void __iomem *virt_baseptr; /* stores ioremap output */
...
unsigned long sec_hal_mem_msg_area_read(void *dst, const void *src,
			unsigned long sz);
void __iomem *sec_hal_mem_msg_area_calloc(unsigned int n, unsigned int sz);
void sec_hal_mem_msg_area_free(void __iomem *vaddr);
...
static void sec_hal_mem_msg_area_memset(
	...
	unsigned int cnt)
{
	__u8 __iomem *ptr = buff;

	while (cnt > 0) {
		writeb_relaxed(data, ptr);
		ptr++;
		cnt--;
	}
}
...
int sec_hal_icram0_init(void)
{
	...
	for (; idx < BLOCKCOUNT; idx++) {
		g_smc0.msg_blocks[idx].virt_addr =
		  (g_smc0.virt_baseptr + blk_sz*idx);
		g_smc0.msg_blocks[idx].size = 0;
		g_smc0.msg_blocks[idx].alloc = FALSE;
	}
