...
long sec_hal_cnf_ioctl(unsigned int cmd, void **data,
			sd_ioctl_params_t *param, struct platform_device *pdev);
...
static uint32_t _siml_verify_data_get(sd_ioctl_params_t *p)
{
	...
	if (SEC_HAL_RES_OK == rv && p->param2
		&& copy_to_user((void __user *)p->param2, hashes[0], len))
		rv = SEC_HAL_RES_FAIL;
	if (SEC_HAL_RES_OK == rv && p->param3
		&& copy_to_user((void __user *)p->param3, hashes[1], len))
		rv = SEC_HAL_RES_FAIL;
	if (SEC_HAL_RES_OK == rv && p->param4
		&& copy_to_user((void __user *)p->param4, hashes[2], len))
		rv = SEC_HAL_RES_FAIL;
	if (SEC_HAL_RES_OK == rv && p->param5
		&& copy_to_user((void __user *)p->param5, hashes[3], len))
		rv = SEC_HAL_RES_FAIL;
	if (SEC_HAL_RES_OK == rv && p->reserved1
		&& copy_to_user((void __user *)p->reserved1, hashes[4], len))
		rv = SEC_HAL_RES_FAIL;
	if (SEC_HAL_RES_OK == rv && p->param1 /* master_code_data */
		&& copy_to_user((void __user *)p->param1, hashes[5], len))
		rv = SEC_HAL_RES_FAIL;
