...
int sec_hal_rpc_write(struct file *filp, const char __user *buf,
			size_t count, loff_t *ppos);
long sec_hal_rpc_ioctl(unsigned int cmd, void **data,
	...
	loff_t *ppos)
{
	...
	cnt = copy_from_user(&param, (const void __user *)buf,
			SD_RPC_PARAMS_SZ);
