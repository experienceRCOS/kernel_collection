...
#ifdef CONFIG_MOBICORE_API
	union fc_generic mem_reserve;

	memset(&mem_reserve, 0, sizeof(mem_reserve));

	mem_reserve.as_in.cmd = MC_FC_DYNAMIC_MEMORY_ALLOC;
	mem_reserve.as_in.param[0] = area_id;
	mem_reserve.as_in.param[1] = phys_start_addr;
	mem_reserve.as_in.param[2] = phys_end_addr;

	pr_debug("security: protect mem %x %x %x %x\n",
			mem_reserve.as_in.cmd,
			mem_reserve.as_in.param[0],
			mem_reserve.as_in.param[1],
			mem_reserve.as_in.param[2]
	      );
	_smc(&mem_reserve);

	sec_api_st = mem_reserve.as_out.ret;
	pr_debug("security: protect mem done %x\n",
			mem_reserve.as_out.ret
	      );

	if (SEC_HAL_CMN_RES_OK != sec_api_st) {
		SEC_HAL_TRACE("returned a fail!! serv == %d", sec_api_st);
		sec_api_st = SEC_HAL_CMN_RES_FAIL;
	}
#else
	uint32_t sec_msg_st = SEC_MSG_STATUS_OK;
...
#ifdef CONFIG_MOBICORE_API
	union fc_generic mem_fr;

	memset(&mem_fr, 0, sizeof(mem_fr));

	mem_fr.as_in.cmd = MC_FC_DYNAMIC_MEMORY_FREE;
	mem_fr.as_in.param[0] = area_id;
	mem_fr.as_in.param[1] = 0;
	mem_fr.as_in.param[2] = 0;

	pr_debug("security: unprotect mem %x %x %x %x\n",
			mem_fr.as_in.cmd,
			mem_fr.as_in.param[0],
			mem_fr.as_in.param[1],
			mem_fr.as_in.param[2]
	      );
	_smc(&mem_fr);

	sec_api_st = mem_fr.as_out.ret;

	pr_debug("security: unprotect mem done %x\n",
			mem_fr.as_out.ret
	      );
	if (SEC_HAL_CMN_RES_OK != sec_api_st) {
		SEC_HAL_TRACE("[%s] returned a fail!! serv == %d",
			__func__, sec_api_st);
		sec_api_st = SEC_HAL_CMN_RES_FAIL;
	}
#else
...
#ifdef CONFIG_MOBICORE_API
	union fc_generic dbg_reg;

	memset(&dbg_reg, 0, sizeof(dbg_reg));

	dbg_reg.as_in.cmd = MC_FC_DEBUG_MODE_SET;
	dbg_reg.as_in.param[0] = *dbgreg1;
	dbg_reg.as_in.param[1] = *dbgreg2;
	dbg_reg.as_in.param[2] = *dbgreg3;

	_smc(&dbg_reg);

	sec_api_st = dbg_reg.as_out.ret;
...
/* ****************************************************************************
 * Function name      : sec_hal_arbmode_set
 * Description        : Set ARB mode value to TZ.
 *
 * Parameters         : IN  --- uint32_t *arbmode, pointer to in value.
 *
 * Return value       : uint32
 *                      ==SEC_HAL_CMN_RES_OK operation successful
 *                      failure otherwise.
 * ***************************************************************************/
uint32_t sec_hal_arbmode_set(uint32_t *arbmode)
{
	uint32_t sec_api_st = SEC_HAL_CMN_RES_OK;

#ifdef CONFIG_MOBICORE_API
	union fc_generic arb_mode;

	memset(&arb_mode, 0, sizeof(arb_mode));

	arb_mode.as_in.cmd = MC_FC_ARB_MODE_SET;
	arb_mode.as_in.param[0] = *arbmode;
	arb_mode.as_in.param[1] = 0;
	arb_mode.as_in.param[2] = 0;

	_smc(&arb_mode);

	sec_api_st = arb_mode.as_out.ret;
	...
	do {
		...
		/* write content to the input msg */
		if (sec_msg_param_write32(&in_handle,
				arbmode ? *arbmode : SEC_HAL_CMN_ARBMODE_COUNTER,
				SEC_MSG_PARAM_ID_NONE) != SEC_MSG_STATUS_OK) {
			SEC_HAL_TRACE("arbmode write error, aborting!");
			sec_api_st = SEC_HAL_CMN_RES_FAIL;
			break;
		}
		LOCAL_WMB();
		...
		/* call dispatcher */
		ssa_disp_st = raw_pub2sec_dispatcher(SEC_SERV_ARBMODE_SET,
			LOCAL_DEFAULT_DISP_FLAGS,
			LOCAL_DEFAULT_DISP_SPARE_PARAM,
			SEC_HAL_MEM_VIR2PHY_FUNC(out_msg),
			SEC_HAL_MEM_VIR2PHY_FUNC(in_msg));
