...
#ifndef GPIO_LOW_BAT_DET
#define GPIO_LOW_BAT_DET 1
#endif
...
#define STC311x_DELAY	msecs_to_jiffies(500)
...
void STC311x_Reset(void)
{
	...
	/* The first reported SOC value could be invalid data because STC3115 require some delaytime 2 seconds
	   for stabilizing internal current sensor and temperature sensor after GasGauge_Start().
	   So 2 seconds delay function(msleep(2000)) need to be inserted between GasGauge_Start()
	   and GasGauge_Task() in  GasGauge_Start() in stc311x_probe() function.*/
...
static irqreturn_t stc311x_irq_handler(int irq, void *dev)
{
	...
	schedule_delayed_work(&chip->irq_work, STC311x_DELAY); 
	return IRQ_HANDLED;
}
#endif
