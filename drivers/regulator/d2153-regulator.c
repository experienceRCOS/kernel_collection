...
static int d2153_regulator_enable(struct regulator_dev *rdev)
{
	...
	if(regulator_id == D2153_LDO_12)
	{
		d2153_reg_write(d2153, 0x55, 0x44);
		d2153_reg_write(d2153, 0x5A, 0x44);
		return 0;
	}
#endif
...
static int d2153_regulator_disable(struct regulator_dev *rdev)
{
	...
	if(regulator_id == D2153_LDO_12)
	{
		d2153_reg_write(d2153, 0x5A, 0x00);	      
		d2153_reg_write(d2153, 0x55, 0x00);
		return 0;
	}
#endif
	if (!is_mode_control_enabled()) {
